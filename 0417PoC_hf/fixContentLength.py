...
def process_request_file(raw_file: Path, root_dir: Path) -> None:
...
def process_directory(root_dir: Path) -> None:
    ...
    for raw_file in raw_files:
        process_request_file(raw_file, root_dir)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Fix Content-Length in *.request.raw files. "
    ...
    parser.add_argument(
        ...
        type=Path,
        default=SCRIPT_DIR,
        help=f"Target directory or .request.raw file (default: {SCRIPT_DIR})",
    )
    return parser
...
if __name__ == "__main__":
    ...
    if target_path.is_file():
        process_request_file(target_path, target_path.parent)
        sys.exit(0)
    if not target_path.is_dir():
        print(f"错误: '{target_path}' 不是一个目录或 .request.raw 文件")
        sys.exit(1)
