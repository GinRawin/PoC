...
漏洞名称：
Netgear WNDR37AVv2 1.0.0.10 strcpy NULL 指针解引用漏洞
...
第一步。请求命中 apply.cgi 路径，trace 进入 post_apply/do_apply -> cgi_setobject

第二步。cgi_setobject 读取 submit_flag，在 obj.funcs 中匹配到 "pptp"，调用 0x40ad34

第三步。0x40ad34 读取 DNSAssign 失败得到 NULL，随后执行 strcpy(sp+0x30, NULL) 触发 SIGSEGV
