...
Netgear WNR2500 1.0.0.24 0x405344 空指针解引用漏洞
...
第一步。handle_request 解析 POST /restore.cgi HTTP/1.1，把路径规范化为 restore.cgi，随后在 0x408304 调用 strchr(s1, '?') 检查 query。由于请求 URI 不含 ?，v71是NULL。接着，handle_request函数将v51赋值为v71并作为restore.cgi对应的处理函数0x405344的第一个参数，在0x405378处将第一个函数参数传入strtr，出现空指针解引用
![alt text](image.png)
