...
漏洞名称：
Netgear WNDRMAC 1.0.0.10 0x43e368 空指针解引用漏洞
...
第一步。POST /apply.cgi? 进入 uhttpd 的通用 CGI 分发路径，cgi_setobject 在 0x40f7b0 读取 submit_flag

第二步。submit_flag=edit_qos_mac 使控制流命中 sym.config_edit_qos_mac (0x43ebb0)，而不是 request.handler_name 里的 passphrase_bg_msg

第三步。sym.config_edit_qos_mac 调用 fcn.0043e314，其中 0x43e358 读取 select_editnum_mac，sym.cgi_value 在 0x40e3cc 返回 NULL

第四步。fcn.0043e314 在 0x43e368 调用 atoi(NULL)，trace 落到 0x43e360 -> 0x44b690 后以 SIGSEGV si_addr=NULL 结束
