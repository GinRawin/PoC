...
漏洞名称：
Netgear XWN5001 0.4.1.1 make_funcsjs 栈缓冲区溢出漏洞
...
该漏洞位于二进制文件 `/usr/sbin/uhttpd` 中。请求进入 `handle_request` 后，HTTP 请求行中的第二个 token 会被解析为请求目标路径，并在 `0x409c80` 处作为参数传给 `make_funcsjs`。`make_funcsjs` 在 `0x405c24` 调用 `sprintf`，将攻击者可控路径写入位于 `sp+0x18` 的局部栈缓冲区。由于该操作没有长度检查，超长路径可覆盖返回状态，并在函数尾声 `0x405cf8` 附近触发崩溃。
...
第一步。攻击者向目标设备发送超长 GET 请求，请求路径以 `/` 开头并包含异常长的 handler 名称。`uhttpd` 在 `handle_request` 中读取请求行后，使用 `strsep` 提取第二个 token 作为请求目标，并在后续调度阶段把该路径直接传给 `make_funcsjs` 处理。
![alt text](image-1.png)
![alt text](image-2.png)

第二步。程序在 `make_funcsjs` 中使用 `sprintf(sp+0x18, format, attacker_path)` 将攻击者可控路径写入栈缓冲区。由于缓冲区长度固定且不存在边界检查，超长输入覆盖了保存寄存器和返回地址，函数返回阶段发生 `SIGSEGV`，崩溃地址呈现 `0x61616160`，说明覆盖内容来自输入中的连续 `'a'` 字节。
![alt text](image.png)
