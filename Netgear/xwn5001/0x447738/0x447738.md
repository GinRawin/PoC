...
The call to strcpy at address `0x447738` in the function `sub_447690`. 
...
2. After sending the second data packet, since the value of `submit_flag` is `wadv_schedule_edit`, the program calls `sub_447690` via the `off_4718B0` function table in the `cgi_setobject` function. In `sub_447690`, the program obtains a variable based on the value of `wladv_schedule_edit_num`. After checking its value is bigger than 0, the program extracts value of `schedule_info` from the package and propagates its value to `strcpy` function, which causes a buffer overflow.
