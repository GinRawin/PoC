...
The call to strcpy at address `0x42C408` in the function `sub_42C2E0`. 
...
2. Send the second data packet. In function `ExecuteSoapAction`, the parameter `WANIPConnection:#SetIPInterfaceInfo` will call the function `sub_42CE44` based on `off_458430`  in `SOAPActions`. The value of `wan_gateway` is set to the `NewDefaultGateway` value from the packet.

![alt text](image.png)

![alt text](image-7.png)

3. Send the third data packet. In function `ExecuteSoapAction`, the parameter `WANIPConnection:#GetInfo` will call the function `sub_42C2E0` based on `off_458430`  in `SOAPActions`.  Finally, upon reaching address `0x42C408`, the call to the `strcpy` function **triggered a buffer overflow.**
